...
export function PreviewGoogleDriveSetup({ onNavigate }: { onNavigate: (path: string) => void }): React.ReactElement {
  ...
  return <section className="route-view preview-drive-setup" aria-labelledby="preview-drive-title">
    <div><span className="workspace-eyebrow">GOOGLE WORKSPACE · FUTURE MIGRATION TARGET</span><h2 id="preview-drive-title">회사 Google Drive 연결</h2><p>현재 Cloudflare 검증 중인 프로젝트 파일은 D1 임시 저장소에 보존됩니다. 회사 계정이 확정되면 관리자가 언제든 연결 계정을 변경하고 프로젝트별 폴더로 이관할 수 있습니다.</p></div>
    <div className="preview-drive-status" role="status"><span className={status?.connected ? 'is-connected' : ''}>{status?.connected ? 'CONNECTED' : 'DEFERRED'}</span><strong>{status?.connected ? `회사 계정 연결됨${status.accountEmail ? ` · ${status.accountEmail}` : ''}` : status?.configured ? 'OAuth 설정 준비됨 · 관리자 동의 전' : '사용자 요청에 따라 Google Drive 연결 보류 중'}</strong></div>
    ...
    {error && <p className="error-box" role="alert">{error}</p>}
    <div className="preview-drive-actions"><div><button type="button" disabled={busy || !status?.configured} onClick={() => void connect()}>{busy ? '처리 중…' : status?.connected ? '다른 회사 Google 계정으로 변경' : '회사 Google 계정 연결'}</button>{status?.connected && <button type="button" disabled={busy} onClick={() => void disconnect()}>연결 해제</button>}<Button variant="secondary" onClick={() => onNavigate('/cases/files')}>현재 자료실 보기</Button></div><span>현재 저장 · D1 TEMPORARY · R2 미사용</span></div>
  </section>;
}
