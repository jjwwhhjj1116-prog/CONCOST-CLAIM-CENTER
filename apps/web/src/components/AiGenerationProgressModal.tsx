...
export function AiGenerationProgressModal({
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setElapsedSeconds((current) => {
        ...
        setProgress(Math.min(98, 8 + Math.round((next / timeoutHintSeconds) * 90)));
        return next;
      });
    }, 1_000);
  ...
  return <div className="ai-generation-overlay" role="presentation">
    <section className={`ai-generation-modal is-${status}`} role="dialog" aria-modal="true" aria-labelledby="ai-generation-title" aria-describedby="ai-generation-description">
      ...
      <div className="ai-generation-modal__meter" role="progressbar" aria-label={`${statusLabel} 예상 진행률`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={status === 'error' ? progress : status === 'complete' ? 100 : progress} aria-valuetext={status === 'running' ? `예상 ${progress}% · ${elapsedSeconds}초 경과` : statusLabel}>
        <span><i style={{ width: `${status === 'complete' ? 100 : progress}%` }} /></span>
        <strong>{status === 'error' ? '중단' : status === 'complete' ? '100%' : `예상 ${progress}% · ${elapsedSeconds}초`}</strong>
      </div>
