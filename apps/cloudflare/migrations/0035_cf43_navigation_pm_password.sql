...
WHEN NOT EXISTS (
  ...
  WHERE c.id=NEW.case_id AND c.organization_id=NEW.organization_id AND c.deleted_at IS NULL
    ...
    AND NOT EXISTS (SELECT 1 FROM json_each(pm.roles_json) r WHERE lower(r.value)='admin')
...
) BEGIN SELECT RAISE(ABORT,'schedule profile requires one of the five approved non-Admin PMs'); END;
...
BEGIN SELECT RAISE(ABORT,'schedule profile update requires one of the five approved non-Admin PMs'); END;
