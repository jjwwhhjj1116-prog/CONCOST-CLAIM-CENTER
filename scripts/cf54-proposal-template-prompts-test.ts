...
test('CF54 stores isolated admin-only prompt profiles for every proposal template',async()=>{
  ...
  const admin=await adminResponse.json() as {phase:string;promptProfiles:Array<{templateSourceId:string;templateSourceName:string;templateCategory:string;version:number;chapters:Array<{chapterNumber:number;executionOrder:number;instructionText:string;version:number}>}>};
  assert.equal(admin.phase,'CF54_PROPOSAL_TEMPLATE_PROMPT_PROFILES');assert.equal(admin.promptProfiles.length,23);assert.ok(admin.promptProfiles.every((profile)=>profile.chapters.length===3));
...
test('CF54 proposal AI generation is explicitly chained 2 to 1 to 3 and then validated',()=>{
  ...
  assert.ok(chapter2>0&&chapter1>chapter2&&chapter3>chapter1&&validation>chapter3);assert.match(source,/PROPOSAL_AI_VALIDATION_FAILED/u);assert.match(source,/aiGenerationTrace/u);assert.match(source,/promptProfile\.templateCategory/u);
  assert.match(source,/reasoningEffort: 'low'/u);assert.match(source,/organizationGemini,40_000/u);
  const ui=read('apps/web/src/proposals/ProposalView.tsx');assert.match(ui,/timeoutMs:generationMode==='AI'\?180_000:30_000/u);
  const progress=read('apps/web/src/components/AiGenerationProgressModal.tsx');assert.match(progress,/180초를 넘기면 원인을 표시하고 자동 중단/u);assert.doesNotMatch(progress,/Math\.min\(92/u);
});
