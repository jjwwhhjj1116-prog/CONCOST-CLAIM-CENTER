...
test('CF78 routes and responsive UI expose contacts to members and DB management only to admins',()=>{
  ...
  assert.match(router,/CONTACT-03[\s\S]*allowedRoles: ADMIN_ONLY/u);assert.match(shell,/인맥관리[\s\S]*CONTACT-03/u);assert.match(ui,/Gemini 구조화 인식/u);assert.match(ui,/Google Drive 원본 저장/u);assert.match(css,/@media\(max-width:720px\)/u);
});
