...
export async function startP16Isolated(testName: string, customOptions = {}): Promise<P16TestContext> {
  ...
  const backupRootDir = path.join(root, 'packages/database/.data', `${unique}-backups`);
  const restoreRootDir = path.join(root, 'packages/database/.data', `${unique}-restores`);
  const databaseUrl = `file:${databasePath}`;
  const backupSigningKey = crypto.createHash('sha256').update(`P16 key ${unique}`).digest();
  ...
  const cleanup = async () => {
    ...
    try { if (fs.existsSync(backupRootDir)) fs.rmSync(backupRootDir, { recursive: true, force: true }); } catch {}
  };
