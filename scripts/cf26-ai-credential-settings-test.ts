...
test('CF26 uses personal key before organization and environment, then falls back after disable', async () => {
  ...
  configResponse=await worker.fetch(request(`/api/report-authoring/config?caseId=${CASE_ID}`,STAFF_TOKEN),env); config=await configResponse.json() as {credentialSource:string;assistantConnected:boolean;assistantCredentialSource:string;chapters:Array<{id:string}>}; assert.equal(config.credentialSource,'ORGANIZATION'); assert.equal(config.assistantConnected,false); assert.equal(config.assistantCredentialSource,'NONE');
  const blockedImprove=await worker.fetch(request('/api/report-authoring/improve',STAFF_TOKEN,{method:'POST',body:JSON.stringify({caseId:CASE_ID,content:'조직 키로 대신 실행되면 안 됩니다.',instruction:'문장을 더 명확하게 개선해 주세요.',expectedDraftVersion:0})}),env); assert.equal(blockedImprove.status,503); assert.equal((await blockedImprove.json() as {code:string}).code,'PERSONAL_GEMINI_NOT_CONFIGURED');
