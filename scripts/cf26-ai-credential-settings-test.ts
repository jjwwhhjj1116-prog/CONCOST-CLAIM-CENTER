...
test('CF26 uses personal key before organization and environment, then falls back after disable', async () => {
  ...
  let configResponse=await worker.fetch(request(`/api/report-authoring/config?caseId=${CASE_ID}`,STAFF_TOKEN),env); let config=await configResponse.json() as {credentialSource:string;chapters:Array<{id:string}>}; assert.equal(config.credentialSource,'PERSONAL');
  ...
  configResponse=await worker.fetch(request(`/api/report-authoring/config?caseId=${CASE_ID}`,STAFF_TOKEN),env); config=await configResponse.json() as {credentialSource:string;chapters:Array<{id:string}>}; assert.equal(config.credentialSource,'ORGANIZATION');
