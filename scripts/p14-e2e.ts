...
function productionWebServer(webOrigin: string): Server {
  ...
  if (!fs.existsSync(path.join(distRoot, 'index.html'))) {
    throw new Error('P14 browser E2E requires current production build in apps/web/dist');
  }
...
async function main() {
  console.log('--- Starting P14 Chromium Real E2E Test ---');
  const unique = `p14-e2e-${process.pid}`;
  ...
  try {
    ...
    // 1. Admin Login & Google Workspace Integration Setup
    console.log('1. Admin login & Google Workspace connection...');
    ...
    // Click Connect
    await adminPage.getByRole('button', { name: 'Google Workspace 동의 & 연동' }).click();
    await adminPage.waitForSelector('text=연결됨 (CONNECTED)');

    // Test Connection
    console.log('2. Testing Google Workspace connection health...');
    await adminPage.getByRole('button', { name: '연결 상태 테스트' }).click();
    await adminPage.waitForSelector('text=연결 테스트 성공');

    // Disconnect & Data Preservation
    console.log('3. Testing Disconnect & internal data preservation...');
    adminPage.on('dialog', (dialog) => void dialog.accept());
    await adminPage.getByRole('button', { name: '연동 해제' }).click();
    await adminPage.waitForSelector('text=해제됨 (DISCONNECTED)');

    console.log('✅ P14 Chromium Real E2E Test Passed Cleanly!');
  } finally {
...
main().catch((err) => {
  console.error('❌ P14 Chromium E2E Test Failed:', err);
  process.exit(1);
});
